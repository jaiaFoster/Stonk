...
def config_snapshot(run_mode: str) -> dict[str, Any]:
    return {
        ...
        "ff_dev_cap_discovery_slots": getattr(config, "FF_DEV_CAP_DISCOVERY_SLOTS", 0),
        "ff_near_miss_window": getattr(config, "FF_NEAR_MISS_WINDOW", 0.05),
        "ff_excluded_tickers": sorted(getattr(config, "FF_EXCLUDED_TICKERS", set())),
        "ff_min_leg_open_interest": config.FF_MIN_LEG_OPEN_INTEREST,
        "ff_min_leg_volume": config.FF_MIN_LEG_VOLUME,
        "ff_max_leg_bid_ask_pct": config.FF_MAX_LEG_BID_ASK_PCT,
        "ff_max_package_slippage_pct": config.FF_MAX_PACKAGE_SLIPPAGE_PCT,
        "ff_warn_package_slippage_pct": config.FF_WARN_PACKAGE_SLIPPAGE_PCT,
...
def config_log_lines(snapshot: dict[str, Any]) -> list[str]:
    lines = [
        ...
        f"FF_DEV_CAP_DISCOVERY_SLOTS: {snapshot.get('ff_dev_cap_discovery_slots')}",
        f"FF_NEAR_MISS_WINDOW: {snapshot.get('ff_near_miss_window')}",
        f"FF_EXCLUDED_TICKERS: {snapshot.get('ff_excluded_tickers')}",
        f"FF_LIQUIDITY: OI>={snapshot.get('ff_min_leg_open_interest')} vol>={snapshot.get('ff_min_leg_volume')} spread<={snapshot.get('ff_max_leg_bid_ask_pct')}% slippage<={snapshot.get('ff_max_package_slippage_pct')}% warn={snapshot.get('ff_warn_package_slippage_pct')}%",
