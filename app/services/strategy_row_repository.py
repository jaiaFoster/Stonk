...
class StrategyRowRepository:
        ...
        with self._connect() as conn:
            conn.execute(
                ...
                CREATE TABLE IF NOT EXISTS strategy_rows (
                    ...
                    build_eligible INTEGER,
                    surface_eligible INTEGER,
                    entry_evaluation_eligible INTEGER,
                    entry_allowed INTEGER,
                    terminal INTEGER,
                    policy_version TEXT,
                    disposition_code TEXT,
                    disposition_reason TEXT,
                    current_structure_id TEXT,
                    structure_version INTEGER,
                    previous_structure_id TEXT,
                    structure_changed INTEGER,
                    structure_change_reason TEXT,
        ...
        with self._connect() as conn:
            ...
            conn.executemany(
                ...
                INSERT OR REPLACE INTO strategy_rows (
                    ...
                    opportunity_id,lifecycle_stage,strategy_stage,calendar_stage,evaluation_state,trade_verdict,
                    recommended_action,build_eligible,surface_eligible,entry_evaluation_eligible,entry_allowed,
                    terminal,policy_version,disposition_code,disposition_reason,current_structure_id,
                    structure_version,previous_structure_id,structure_changed,structure_change_reason,
    ...
    def _normalize_for_store(
        ...
        return {
            ...
            **semantics,
            "opportunity_id": compact.get("opportunity_id"),
            "lifecycle_stage": compact.get("lifecycle_stage"),
            "strategy_stage": compact.get("strategy_stage"),
            "calendar_stage": compact.get("calendar_stage"),
            "evaluation_state": compact.get("evaluation_state"),
            "trade_verdict": compact.get("trade_verdict"),
            "recommended_action": compact.get("recommended_action"),
            "build_eligible": 1 if compact.get("build_eligible") else 0,
            "surface_eligible": 1 if compact.get("surface_eligible") else 0,
            "entry_evaluation_eligible": 1 if compact.get("entry_evaluation_eligible") else 0,
            "entry_allowed": 1 if compact.get("entry_allowed") else 0,
            "terminal": 1 if compact.get("terminal") else 0,
            "policy_version": compact.get("policy_version"),
            "disposition_code": compact.get("disposition_code"),
            "disposition_reason": compact.get("disposition_reason"),
            "current_structure_id": compact.get("current_structure_id"),
            "structure_version": int(compact.get("structure_version") or 0) if compact.get("structure_version") is not None else None,
            "previous_structure_id": compact.get("previous_structure_id"),
            "structure_changed": 1 if compact.get("structure_changed") else 0,
            "structure_change_reason": compact.get("structure_change_reason"),
    ...
    def _row_to_api(row: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            "calendar_stage": row.get("calendar_stage"),
            "evaluation_state": row.get("evaluation_state"),
            "trade_verdict": row.get("trade_verdict"),
            "recommended_action": row.get("recommended_action"),
            "build_eligible": bool(row.get("build_eligible")),
            "surface_eligible": bool(row.get("surface_eligible")),
            "entry_evaluation_eligible": bool(row.get("entry_evaluation_eligible")),
            ...
            "current_structure_id": row.get("current_structure_id"),
            "structure_version": row.get("structure_version"),
            "previous_structure_id": row.get("previous_structure_id"),
            "structure_changed": bool(row.get("structure_changed")),
            "structure_change_reason": row.get("structure_change_reason"),
